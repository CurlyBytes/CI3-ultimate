  	...
  	foreach ($this->config['handlers'] as $value)
		{
			switch ($value)
			{
				...
				case 'ci_file':
					...
					$handler->setFormatter($formatter);
					//$configchrom = array('message' => "%level_name% - %datetime% --> %message% %extra%\n");
					//$formatter = new \Monolog\Formatter\ChromePHPFormatter($configchrom);
					//$handler->setFormatter($formatter);
					break;
				...
				case 'stderr':
					...
					$handler->setFormatter(new \Bramus\Monolog\Formatter\ColoredLineFormatter(new Bramus\Monolog\Formatter\ColorSchemes\TrafficLight()));
					break;
				...
				case 'phpconsole':
					$handler = new \Monolog\Handler\PHPConsoleHandler(array() );				
					break;
